...
def FAR_show_sample(VPTR_Enc, VPTR_Dec, VPTR_Transformer, num_pred, sample, save_dir, test_phase = True):
    ...
    # Adjust the slicing to ensure dimensions match
    ...
    visualize_batch_clips(past_frames[0:idx, 1:, ...], pred_past_frames[0:idx, :, ...], pred_future_frames[0:idx, :, ...], save_dir, renorm_transform, desc = 'pred_past')

if __name__ == '__main__':
    set_seed(2021)
    ckpt_save_dir = Path('D:\VPTR_jigsaws\jigsaws_suturing\JIGSAWS_FAR_MSEGDLgan_ckpt')
    tensorboard_save_dir = Path('D:\VPTR_jigsaws\jigsaws_suturing\JIGSAWS_FAR_MSEGDLgan_tensorboard')
    resume_AE_ckpt = Path('D:\VPTR_jigsaws\jigsaws_suturing\JIGSAWS_ResNetAE_MSEGDLgan_ckpt').joinpath('epoch_1.tar')
    ...
    #####################Init Dataset ###########################
    data_set_name = 'BAIR'
    dataset_dir = r"D:\VPTR_jigsaws\jigsaws_suturing\bair_format_dir"
    test_past_frames = 2
    ...
    VPTR_Dec = VPTRDec(img_channels, feat_dim = encC, n_downsampling = 3, out_layer = 'Tanh').to(device) #Tanh for KTH and BAIR
