...
def get_dataloader(data_set_name, batch_size, data_set_dir, test_past_frames = 10, test_future_frames = 10, ngpus = 1, num_workers = 1):
    ...
    elif data_set_name == 'JIGSAWSDataset':
        transform = transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize(mean = [0.485, 0.456, 0.406], std = [0.229, 0.224, 0.225])
        ])
        dataset = SuturingDataset(data_path=data_set_dir, transform=transform)
        train_val_split = int(0.8 * len(dataset))
        train_set, val_set = random_split(dataset, [train_val_split, len(dataset) - train_val_split])
