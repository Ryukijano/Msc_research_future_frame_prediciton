...
def cal_lossD(VPTR_Disc, fake_imgs, real_imgs, lam_gan):
    # ... (Your existing cal_lossD function) ...

def cal_lossG(VPTR_Disc, fake_imgs, real_imgs, mu, logvar, lam_gan, beta=1.0):
    ...
    KLD_loss /= fake_imgs.shape[0]  # Normalize by batch size

    loss_G = lam_gan * loss_G_gan + AE_MSE_loss + AE_GDL_loss + beta * KLD_loss

    return loss_G, loss_G_gan, AE_MSE_loss, AE_GDL_loss, KLD_loss
...
def single_iter(VPTR_Enc, VPTR_Dec, VPTR_Disc, optimizer_G, optimizer_D, sample, device, train_flag=True):
    ...
    if train_flag:
        ...
        mu, logvar = VPTR_Enc(x)  # Get mean and log variance from the encoder
        z = reparameterize(mu, logvar)  # Sample latent vector
        rec_frames = VPTR_Dec(z)  # Decode the latent vector
        ...
        loss_G, loss_G_gan, AE_MSE_loss, AE_GDL_loss, KLD_loss = cal_lossG(VPTR_Disc, rec_frames, x, mu, logvar, lam_gan) # Pass mu and logvar to cal_lossG
        loss_G.backward()
        optimizer_G.step()

        iter_loss_dict = {'AEgan': loss_G_gan.item(), 'AE_MSE': AE_MSE_loss.item(), 'AE_GDL': AE_GDL_loss.item(),
                         'AE_total': loss_G.item(), 'Dtotal': loss_D.item(), 'Dfake': loss_D_fake.item(),
                         'Dreal': loss_D_real.item(), 'KLD': KLD_loss.item()}
    else:
...
def show_samples(VPTR_Enc, VPTR_Dec, sample, save_dir, renorm_transform):
    ...
    with torch.no_grad():
        ...
        idx = min(N, 4)
        visualize_batch_clips(past_frames[0:idx], future_frames[0:idx], rec_frames[0:idx], save_dir,
                              renorm_transform, desc='ae')

if __name__ == '__main__':
    ckpt_save_dir = Path('C:\\VPTR_jigsaws\\jigsaws_suturing\\VPTR_ckpts\\JIGSAWS_ResnetAE_MSEGDLgan_ckpt')
    tensorboard_save_dir = Path('C:\\VPTR_jigsaws\\jigsaws_suturing\\VPTR_ckpts\\JIGSAWS_ResnetAE_MSEGDLgan_tensorboard')
    ...
    #####################Init Dataset ###########################
    data_set_name = 'Suturing' #see utils.dataset
    dataset_dir = 'C:\\VPTR_jigsaws\\jigsaws_suturing\\frames_split\\'
    train_loader, test_loader, renorm_transform = get_dataloader(data_set_name, N, dataset_dir, num_past_frames, num_future_frames)
    ...
    #####################Init Criterion ###########################
    loss_name_list = ['AE_MSE', 'AE_GDL', 'AE_total', 'Dtotal', 'Dfake', 'Dreal', 'AEgan', 'KLD']
