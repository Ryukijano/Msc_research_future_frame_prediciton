...
def show_samples(VPTR_Enc, VPTR_Dec, sample, save_dir, renorm_transform):
    ...
    with torch.no_grad():
        ...
        # Concatenate past and future frames
        all_frames = torch.cat([past_frames, future_frames], dim=1)
        
        # Encode and decode all frames
        encoded = VPTR_Enc(all_frames)
        rec_frames = VPTR_Dec(encoded)
        
        # Split reconstructed frames back into past and future
        rec_past_frames = rec_frames[:, :past_frames.shape[1], ...]
        rec_future_frames = rec_frames[:, past_frames.shape[1]:, ...]
        
        N = future_frames.shape[0]
        idx = min(N, 4)
        visualize_batch_clips(past_frames[0:idx, :, ...], 
                              future_frames[0:idx, :, ...], 
                              rec_frames[0:idx, :, ...], 
                              save_dir, renorm_transform, desc='ae')


if __name__ == '__main__':
    ckpt_save_dir = Path("C:\VPTR_jigsaws_modifications\jigsaws_suturing\JIGSAWS_ResNetAE_MSEGDLgan_ckpt")
    tensorboard_save_dir = Path("C:\VPTR_jigsaws_modifications\jigsaws_suturing\JIGSAWS_ResNetAE_MSEGDLgan_tensorboard")

    resume_ckpt = Path("C:\\VPTR_jigsaws_modifications\\jigsaws_suturing\\JIGSAWS_ResNetAE_MSEGDLgan_ckpt\\BAIR_AE.tar")
    #resume_ckpt = None
    start_epoch = 0
    ...
    img_channels = 3 #3 channels for BAIR dataset
    epochs = 1
    N = 8
    AE_lr = 1e-4
    lam_gan = 0.01
    ...
    dataset_dir = r"C:\VPTR_jigsaws_modifications\jigsaws_suturing\bair_format_dir"
